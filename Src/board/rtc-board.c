...
void timestamp_to_time(unsigned int timestamp)
{
	...
	stm = localtime((time_t *)&timestamp);
	RTC_DateStruct.Year = stm->tm_year + 1900 - 2000; // linux since 1900, stm32 since 2000
	RTC_DateStruct.Month = stm->tm_mon + 1;
	RTC_DateStruct.Date = stm->tm_mday;
	if(stm->tm_wday == 0) // must set weekday, otherwise RTC time will not be accuracy
	{
	...
	RTC_TimeStruct.Hours = stm->tm_hour;
	...
	//DebugPrintf("\r\nrtc %d-%d-%d  %d.%d.%d\r\n", 
	//	RTC_DateStruct.Year+2000,RTC_DateStruct.Month,RTC_DateStruct.Date,
...
unsigned int time_to_timestamp(void)
{
	...
	stm.tm_year = RTC_DateStruct.Year + 2000 - 1900;	//RTC_Year rang 0-99,but tm_year since 1900
	...
	stm.tm_hour	= RTC_TimeStruct.Hours;			//RTC_Hours rang 0-23 and tm_hour rang 0-23
